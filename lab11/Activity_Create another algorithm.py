...
# Read in the updated file to verify changes
with open(import_file, "r") as file:
    # Read the file and store its contents in the `text` variable
    text = file.read()

# Display `text` to examine the result
print(text)
...
def update_file(import_file, remove_list):
    ...
    # Convert `ip_addresses` back to a string so that it can be written into the text file 
     ...
     # Convert the contents of the file from a string to a list
    ip_addresses = ip_addresses.split()
    
    # Iterate through the list of IP addresses and remove those in the remove list
    ip_addresses = [ip for ip in ip_addresses if ip not in remove_list]
    
    # Convert the updated list back to a string
    ip_addresses = " ".join(ip_addresses)
    
    # Write the updated contents back into the file
    with open(import_file, "w") as file:
        file.write(ip_addresses)
...
# Call `update_file` with the file name and list of IPs to remove
update_file("allow_list.txt", ["192.168.25.60", "192.168.140.81", "192.168.203.198"])

# Read the updated file and display its contents
...
# Testing the function ensures that the implementation works as expected.
