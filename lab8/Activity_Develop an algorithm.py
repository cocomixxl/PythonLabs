...
# Display the contents of `approved_devoces`
...
if username in approved_users and approved_devices[ind] == device_id:
    print(f"The username {username} is approved to access the system.")
    print(f"{device_id} is the assigned device for {username}.")
else:
    print("The username and device ID do not match or are not approved.")
...
def login(username, device_id):
    # Assign `approved_users` and `approved_devices` to lists
    approved_users = ["elarson", "bmoreno", "sgilmore", "eraab", "gesparza"]
    approved_devices = ["8rp2k75", "hl0s5o1", "4n482ts", "a307vir", "3rcv4w6"]
    
    # Outer conditional: If `username` belongs to `approved_users`
    if username in approved_users:
        print("The user", username, "is approved to access the system.")
        ind = approved_users.index(username)  # Find the index of the username
        
        # Inner conditional: Check the device ID
        if device_id == approved_devices[ind]:
            print(device_id, "is the assigned device for", username)
        else:
            print(device_id, "is not their assigned device.")
    else:
        # If username is not in approved_users
        print("The username", username, "is not approved to access the system.")

# Call the `login` function with different combinations of `username` and `device_id`
login("sgilmore", "4n482ts")  # Expected: User approved, correct device
login("sgilmore", "wrong_id")  # Expected: User approved, wrong device
login("unknown_user", "4n482ts")  # Expected: User not approved
...
# Indexing for Correspondence: Lists (approved_users and approved_devices) must align in order for indexing logic to work.

# In[13]:


Elif for Specific Cases: The "elif" statement helps handle nuanced conditions, improving program feedback.
